 ...
 * @author		Tárcio Zemel <tarciozemel@gmail.com>
 * @version		1.2
...
/*
	...
	-----------------------------------------------------------------------------------------------
		...
		$this->google_load->load(array('jquery', 'jqueryui'));
		
		// Add one library with data returned instead of sent to the output class
		$this->google_load->load('jquery', TRUE);

		// Add multiple libraries with data returned instead of sent to the output class
		$this->google_load->load(array('jquery', 'jqueryui'), TRUE);
...
class Google_load {
    ...
    /**
     * @param array $module_name_string Libraries to load
     * @param boolean $return If 
     * @return string If the $return parameter is TRUE data returned instead of sent to the output class
     */
    function load($module_name_string, $return = FALSE) {
        $time_start = microtime(true);
        $modules = "";
        if (!is_array($module_name_string))
        {
            $module_name_string = array($module_name_string);
        }
        ...
        if ($this->dev) {
            ...
            foreach ($module_name_string as $module_name) {
...
//---------------------------------------------NON DEVELOPMENT---------------------------------------------------//
            ...
            if ($return === TRUE) {
                return "<script" . ' type="text/javascript" src="http://www.google.com/jsapi' . ($this->gl_api ? $api_key = "?key=" . $this->gl_api : '') . '"' . "></script>" . '<script type="text/javascript">' . $modules . "</script>";
